...
for (i in seq_len(nrow(experiments))) {
  ...
  # for L96Dx there are some extra timesteps that need to be filtered out
  if (startsWith(experiment$system, "L96")) {
    experiErrorStats <- experiErrorStats |> filter(nSteps %in% 2^(1:5))
  }
